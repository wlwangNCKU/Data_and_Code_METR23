...
# Generate data
rmnlmm = function(Ti, rXi, rZi, beta, DD, Sigma, phi, ga, N, r)
{
  q = ncol(rZi); p = length(beta); ni = nrow(rXi)
  SigCi = kronecker(Sigma, DEC(phi, ga, Ti))
  b = rmvnorm(N, mean=rep(0, q), sigma=DD)
  e = rmvnorm(N, mean=rep(0, ni), sigma=SigCi)
  A = diag(p)
  B = matrix(c(1,rep(0, 7), 1, 0), ncol=q)
  eta = matrix(rep(A %*% beta, N), nrow=p) + B %*% t(b)
  mu = matrix(NA, nrow=N, ncol=ni)
  for(i in 1: N) mu[i, ] = mu.fn(eta[,i], Ti)$mu
  Ymat = mu + e
  return(list(Ymat=Ymat, y=as.vector(t(Ymat))))
}

rmnlmm.na = function(Ymat, type=c('MAR','MCAR'), na.rate=NULL)
{
  y = as.vector(t(Ymat))
  n = length(y)
  Ni = ncol(Ymat)/2
  y1 = Ymat[,1:Ni]; y2 = Ymat[,-(1:Ni)]
  yy1 = as.vector(t(y1)); yy2 = as.vector(t(y2))
  a = round(n/(2*4))
  if(type=='MAR'){
    quan1 = quantile(yy2, prob=c(0.25, 0.5, 0.75))
    na1 = sort(sample(1:a, a*0.05))
    na2 = sort(sample(1:a, a*0.10))
    na3 = sort(sample(1:a, a*0.15))
    na4 = sort(sample(1:a, a*0.20))
    yy2[which(yy2 < quan1[1])][na1] = NA
    yy2[which(yy2 >= quan1[1] & yy2 < quan1[2])][na2] = NA
    yy2[which(yy2 >= quan1[2] & yy2 < quan1[3])][na3] = NA
    yy2[which(yy2 >= quan1[3])][na4] = NA    
  }
  if(type=='MCAR'){
    n.na = round(n/2 * na.rate)
    na.pos2 = sort(sample((1: (n/2)), n.na))
    yy2[na.pos2] = NA
  }
  Ymat.na = cbind(matrix(yy1, ncol=Ni, byrow=T), matrix(yy2, ncol=Ni, byrow=T))  
  return(list(Ymat=Ymat, Ymat.na=Ymat.na))
}

rmnlmm.cen = function(Ymat, Ymat.na, brna, cen.rate)
{
  Ti = seq(10, 100, 10)
  y = as.vector(t(Ymat))
  n = length(y)
  Ni = length(Ti)
  r = 2
  y1 = Ymat.na[,1:Ni]; y2 = Ymat.na[,-(1:Ni)]
  ubd1 = quantile(y1, prob=cen.rate)
  y1[y1<=ubd1] = ubd1
  cen1 = as.numeric(as.vector(t(y1))==ubd1)
  cen2 = rep(0,n/2)
  cen2[which(is.na(as.vector(t(y2))))] = NA
  
  ycm = as.vector(t(cbind(y1, y2)))
  cen = as.vector(t(cbind(matrix(cen1, N, byrow=T), matrix(cen2, N, byrow=T))))
  sdata = data.frame(cbind(rep(1:N, each=(r*Ni)), rep(rep(Ti, r), N), y, ycm, cen))
  colnames(sdata) = c('Subject', 'Time', 'y', 'Resp', 'Censor')
  gdata = groupedData(Resp~Time|Subject, data=sdata)

  vdata = data.frame(cbind(rep(1: N, each=Ni), rep(Ti, N), as.vector(t(y1)), as.vector(t(y2)), cen1, as.vector(t(Ymat[,1:Ni])), as.vector(t(Ymat[,-(1:Ni)]))))
  colnames(vdata) = c('Subject', 'Time', 'ycm1', 'ycm2', 'cen1', 'y1', 'y2')
  mdata = groupedData(ycm1+ycm2~Time|Subject, data=vdata)
  return(list(Data = mdata, vecData = gdata, Ymat = Ymat, yc=y[which(cen==1)], ym=y[is.na(gdata$Resp)], ubd=ubd1))
}
...
# evaluate new log-likelihood
     ...
     for(i in 2: N){
      Zi = matrix(TrZ[(cumsum.ni[i-1]+1): cumsum.ni[i], ((i-1)*q+1): (i*q)], ncol=q)
      TOme[(cumsum.ni[i-1]+1): cumsum.ni[i], (cumsum.ni[i-1]+1): cumsum.ni[i]] = kronecker(Sigma, DEC(phi,ga, Data$Time[Data$Subject == 1]))
      TLam[(cumsum.ni[i-1]+1): cumsum.ni[i], (cumsum.ni[i-1]+1): cumsum.ni[i]] = Zi %*% DD %*% t(Zi) + TOme[(cumsum.ni[i-1]+1): cumsum.ni[i], (cumsum.ni[i-1]+1): cumsum.ni[i]]    
     }
     TLam.pp = TLam[-na.ind, -na.ind]
     TLam.mp = TLam[na.ind, -na.ind]
...
#     } else log.cdf = log(pmvnorm(lower=-Inf, upper=Utilde, mean=c(mu.co), sigma=Sig.cc.o))[1]
     ...
     if((diff < tol || abs(diff.lnL) < tol) || iter>=max.iter) break
...
### pdf & cdf
  ...
  for(r in 1:(p-1))
  {
    ...
    {
      rs = c(r,s)
      pdf.aa = dmvnorm(c(a[r],a[s]),sigma=R[rs,rs], log =F)
      pdf.ab = dmvnorm(c(a[r],b[s]),sigma=R[rs,rs], log =F)
      pdf.ba = dmvnorm(c(b[r],a[s]),sigma=R[rs,rs], log =F)
      pdf.bb = dmvnorm(c(b[r],b[s]),sigma=R[rs,rs], log =F)
...
# Generate Monte Carlo sample for yc and ym
   ...
   if(!is.na(na.idx[[i]])[1]){ 
      ...
      if(lna > 1) ym.hat = c(rmvnorm(1, mu.mp[na.idx[[i]]], Sig.mm.p[na.idx[[i]], na.idx[[i]]]))
...
# S.beta
   ...
   S[1:p] = S[1:p] + sb    
   SS[1:p, 1:p] = SS[1:p, 1:p] + sb %*% t(sb)  ###不是很???
