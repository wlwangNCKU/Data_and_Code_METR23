...
# missing information
   ...
   cumsum.ni = cumsum(ni)
   oi = numeric(N)
   for(i in 1: N) oi[i] = length(Data$Time[Data$Subject == i])
...
# EM-step:
  ...
  # phi, ga #
     ...
     if(diff.lnL < tol || diff < tol || iter >= max.iter) break
...
# Missing Data Imputation
   pred = IMP.FIT.mnlmm.na(para, b, N, n, p, q, obs.sub, mis.sub, num.obs.sub, num.mis.sub, mis.ind, cumsum.oi, oi)
...
# I_{omega}:
   ...
   for(l in 1: m1)
   {
     ...
     dot.L[[(l-1)*N+1]] = TrZ[1: cumsum.oi[1], 1:q] %*% dot.DD %*% t(TrZ[1: cumsum.oi[1], 1:q])
   ...
   out = rbind(EST, SE)
   FI=list(Itheta = round(FI, 4), Ibeta = FI[1:p, 1:p], Iomega = FI[-(1:p), -(1:p)])
   return(list(out=out, FI=FI))
}

# Imputation and Fitted values:
IMP.FIT.mnlmm.na = function(par, b, N, n, p, q, obs.sub, mis.sub, num.obs.sub, num.mis.sub, mis.ind, cumsum.oi, oi)
{
   Y.na = vecData$Resp
   beta = par$beta
   DD = par$DD
