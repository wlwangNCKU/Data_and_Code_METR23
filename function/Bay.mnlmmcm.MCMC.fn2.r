...
hyper.par = function(par, FI, b)
{
  ...
  return(list(par.mle = par, beta0 = beta0, J0 = J0, d0 = d0, G0 = G0, s0 = s0, H0 = H0, pg.mle = pg.mle, mle.b=b))
}
...
log.pseudo.pos.phiga = function(phi, ga, DD, Sigma, yp.cent, TrZ)
{
   ...
   for(i in 2: N) log.det.Lam.inv = log.det.Lam.inv + log(det(as.matrix(TLam.pp.inv[(cumsum.pi[i-1]+1): cumsum.pi[i], (cumsum.pi[i-1]+1): cumsum.pi[i]])))  
...
MH.phi.nonli = function(phi, DD, Sigma, yp.cent, pg.mle, TrZ, method)
{
   ...
   sd.phi.star = 0.005*sqrt((1/(pg.mle[1]*(1-pg.mle[1])))^2)
...
MH.DEC.nonli = function(phi, ga, DD, Sigma, yp.cent, pg.mle, TrZ, method)
{
   ...
   cov.pg.star = 0.5*matrix(c((1/(pg.mle[1]*(1-pg.mle[1])))^2, 0, 0, 1/pg.mle[2]^2), 2,2)
   repeat{
   phi.ga.star.new = as.vector(rmvnorm(1, phi.ga.star.old, round(cov.pg.star,6)))
   if(phi.ga.star.new[2]>-0.5 & phi.ga.star.new[2]<0.5) break
   }
...
### Gibbs sampler for MNLMM-CM ###
IBF.ycm.tilde.b = function(ITER, hyper)
{
   J = 2 * ITER
   ...
   Zi = matrix(TrZ[1: cumsum.ni[1], 1: q], ncol=q)
   TLam[1: cumsum.ni[1], 1: cumsum.ni[1]] = as.matrix(TrZ[1: cumsum.ni[1], 1:q]) %*% DD %*% t(TrZ[1: cumsum.ni[1], 1:q]) + TOme[1: cumsum.ni[1], 1: cumsum.ni[1]]
   logDetLam = log(det(TLam[1: cumsum.ni[1], 1: cumsum.ni[1]]))
   for(i in 2: N){
    Zi = matrix(TrZ[(cumsum.ni[i-1]+1): cumsum.ni[i], ((i-1)*q+1): (i*q)], ncol=q)
    TOme[(cumsum.ni[i-1]+1): cumsum.ni[i], (cumsum.ni[i-1]+1): cumsum.ni[i]] = kronecker(Sigma, DEC(phi,ga, Data$Time[Data$Subject == i]))
    TLam[(cumsum.ni[i-1]+1): cumsum.ni[i], (cumsum.ni[i-1]+1): cumsum.ni[i]] = Zi %*% DD %*% t(Zi) + TOme[(cumsum.ni[i-1]+1): cumsum.ni[i], (cumsum.ni[i-1]+1): cumsum.ni[i]]    
    logDetLam = logDetLam + log(det(TLam[(cumsum.ni[i-1]+1): cumsum.ni[i], (cumsum.ni[i-1]+1): cumsum.ni[i]]))
   }
   TLam.pp = TLam[-na.ind, -na.ind]
   TLam.mp = TLam[na.ind, -na.ind]
   TLam.mm = TLam[na.ind, na.ind]
   ...
   B = matrix(NA, nrow = J, ncol = N*q)
   Ym.tilde = matrix(NA, nrow = J, ncol = num.na)
   Ytilde.hat = matrix(NA, nrow = J, ncol = n)
   log.pos.den = rep(0, J)
# Yc #
   yo.tilde.cent = Yo.tilde - Xo.tilde %*% beta
   mu.co = Xc.tilde %*% beta + TLam.co %*% TLam.oo.inv %*% yo.tilde.cent
   Sig.cc.o = TLam.cc - TLam.co %*% TLam.oo.inv %*% t(TLam.co)
   Yc.tilde = rtmvnorm(J, mean=c(mu.co), sigma=Sig.cc.o, lower=rep(-Inf, num.cen), upper=Utilde, algorithm="gibbs", burn.in.samples=10)
   Sig.mm.p = TLam.mm - TLam.mp %*% TLam.pp.inv %*% t(TLam.mp)
   TOme.inv = solve(TOme)
   TD.inv = solve(kronecker(diag(N), DD))
   Sb = solve(t(TrZ) %*% TOme.inv %*% TrZ + TD.inv)
   for(j in 1: J){
 # Ym #
     yp.tilde.hat = t(TO) %*% Yo.tilde + t(TC) %*% Yc.tilde[j, ]
     yp.tilde.cent = yp.tilde.hat - Xp.tilde %*% beta
     mu.mp = Xm.tilde%*%beta + TLam.mp%*% TLam.pp.inv %*% yp.tilde.cent
     Ym.tilde[j, ] = c(rmvnorm(1, mu.mp, round(Sig.mm.p,6)))
     Ytilde.hat[j, ] = t(TP) %*% yp.tilde.hat + t(TM) %*% Ym.tilde[j, ]
 # b #
     y.tilde.cent = Ytilde.hat[j, ] - Xtilde%*%beta
     mu.b = Sb %*% t(TrZ) %*% TOme.inv %*% y.tilde.cent
     B[j, ] = c(rmvnorm(1, mu.b, round(Sb, 5)))
 # joint posterior #
     log.pos.den[j] = 0.5*logDetLam + 0.5*N*log(det(D.inv)) - 0.5*t(y.tilde.cent) %*% TLam.inv %*% y.tilde.cent -0.5*t(B[j, ])%*%TD.inv%*%B[j,] + log(dmvnorm(c(beta), beta0, J0) * diwish(DD, d0, G0) * diwish(Sigma, s0, H0)) - 2*log(1+ga)
   ...
   b.IBF = B[ibf.samp, ]
   yc.tilde.IBF = Yc.tilde[ibf.samp, ]
   ym.tilde.IBF = Ym.tilde[ibf.samp, ]
   Ytilde.hat.IBF = Ytilde.hat[ibf.samp, ]
   return(list(b.IBF=b.IBF, yc.til.IBF=yc.tilde.IBF, ym.til.IBF=ym.tilde.IBF, yhat.til.IBF=Ytilde.hat.IBF))
}

IBF.MNLMMCM = function(chain, ITER, hyper, typeCi=c('UNC','AR1','DEC'), nlag=10, per=1000, method=c('pseudo','raw'))
{
   begin=proc.time()[1]
# data
   N = length(unique(vecData$Subject))
...
# save posterior samples:
   ...
   TYhat = array(NA, dim=c(ITER, n, chain))
...
#   TLam.mm = TLam[na.ind, na.ind]
   ...
   cat(paste(rep('-', 20), sep = '', collapse = ''), 'Chain = ', cha, '\t iter = 0, loglik = ', loglik, paste(rep('-', 20), sep = '', collapse = ''), '\n')
   cat('Initial values: beta = ',  round(beta, 2), ',\t D = ', round(DD[vech.DD], 2), '\t Sigma = ', round(Sigma[vech.Sig], 3), ',\t (phi,ga) = ', round(c(phi,ga), 2), '\n')
# Imputation Step:
   IBFsamp = IBF.ycm.tilde.b(ITER, hyper)
   TB[,, cha] = IBFsamp$b.IBF
      
   for(iter in 1: ITER)
   {
# Posterior Step:
 # beta (pseudo data) #
     yp.tilde.hat = IBFsamp$yhat.til.IBF[iter, ][-na.ind]
     TLam.pp.inv = solve(TLam.pp)
     Sig.beta = solve(t(Xp.tilde) %*% TLam.pp.inv %*% Xp.tilde + J0.inv)
     mu.beta = Sig.beta %*% (t(Xp.tilde) %*% TLam.pp.inv %*% yp.tilde.hat + J0.inv %*% beta0)
     beta = c(rmvnorm(1, mu.beta, round(Sig.beta, 6)))
 # DD #
     b = matrix(TB[iter,,cha], nrow=N, byrow=T)
 ...
 # Sigma #
     ...
     Y.hat = IBFsamp$yhat.til.IBF[iter, ] + MU - Xtilde %*% beta - TrZ %*% as.vector(t(b))
     Y.cent = Y.hat - MU
     e.c = NULL
     ...
     if(method=='pseudo'){
     Y.tilde.cent = IBFsamp$yhat.til.IBF[iter, ] - Xtilde %*% beta - TrZ %*%as.vector(t(b))
     e.c = NULL
 ...
 # phi and ga #
     ...
     if(typeCi == 'AR1'){
        ...
        if(method=='pseudo') yp.cent = Y.tilde.cent[-na.ind]
...
# approximate log-likelihood
 ...
 # observed log-likelihood:
     ...
     Sig.cc.o = TLam.cc - TLam.co %*% TLam.oo.inv %*% t(TLam.co)
     if(ci[1]==1){ log.cdf = log(pnorm(Utilde[1:cumsum.nc[1]], mean=mu.co[1:cumsum.nc[1]], sd=sqrt(Sig.cc.o[1:cumsum.nc[1], 1:cumsum.nc[1]])))
     } else log.cdf = log(pmvnorm(lower=rep(-Inf, ci[1]), upper=Utilde[1:cumsum.nc[1]], mean=c(mu.co[1:cumsum.nc[1]]), sigma=round(Sig.cc.o[1:cumsum.nc[1], 1:cumsum.nc[1]],5)))[1]
     if(Nc != 1){
     for(i in 2: Nc){
     if(ci[i]==1){ log.cdf = log.cdf + log(pnorm(Utilde[(cumsum.nc[i-1]+1): cumsum.nc[i]], mean=mu.co[(cumsum.nc[i-1]+1): cumsum.nc[i]], sd=sqrt(Sig.cc.o[(cumsum.nc[i-1]+1): cumsum.nc[i], (cumsum.nc[i-1]+1): cumsum.nc[i]])))
     } else log.cdf = log.cdf + log(pmvnorm(lower=rep(-Inf, ci[i]), upper=Utilde[(cumsum.nc[i-1]+1): cumsum.nc[i]], mean=c(mu.co[(cumsum.nc[i-1]+1): cumsum.nc[i]]), sigma=round(Sig.cc.o[(cumsum.nc[i-1]+1): cumsum.nc[i], (cumsum.nc[i-1]+1): cumsum.nc[i]],5)))[1]
     }}
     log.det.Sig.inv = log(det(TLam.oo.inv[1:cumsum.oi[1], 1:cumsum.oi[1]]))
     for(i in 2: N) log.det.Sig.inv = log.det.Sig.inv + log(det(as.matrix(TLam.oo.inv[(cumsum.oi[i-1]+1): cumsum.oi[i], (cumsum.oi[i-1]+1): cumsum.oi[i]])))
     loglik = log.cdf -0.5*log(2*pi)*no + 0.5*log.det.Sig.inv - 0.5*t(yo.cent.tilde) %*% TLam.oo.inv %*% yo.cent.tilde
